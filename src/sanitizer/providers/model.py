...
class ModelProvider:
    ...
    def _prompt(batch) -> str:
        """Текст запроса к модели. Короткий, английский, по одному варианту на строку.
        ...
        📌 Список «уже занято» убран целиком. Он держался на требовании «ноль склеек»,
        которое отменено (Р-117): занятость теперь предпочтение, а не отказ. До тысячи
        имён в каждом запросе были главным источником шума и денег на входе.
        """
        what = {
