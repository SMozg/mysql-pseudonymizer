...
#: Сколько ячеек идёт в одну фиксацию. ⛔ Не «побольше»: одна транзакция на весь
#: прогон держала бы 5269 блокировок строк и весь откатный журнал до конца, а
#: выигрыш против пачки в пятьсот -- уже в пределах шума.
_COMMIT_EVERY = 500
...
class Applier:
    ...
    def apply(self, cells: Iterable) -> ApplyCounters:
        ...
        try:
            session_init(conn)
            # ⛔ ЗАМЕРЕНО, а не предположено (06.09): применение занимало 48 с из
            # прогона, и 38 из них -- 5269 отдельных UPDATE по 7,27 мс. Причина не
            # в запросах, а в том, что каждый был САМ СЕБЕ ТРАНЗАКЦИЕЙ: соединение
            # открывается с autocommit, то есть на каждую ячейку приходился свой
            # сброс журнала на диск. Логика классификации ячейки НЕ ТРОНУТА ни на
            # строку -- изменилась только частота фиксации.
            # ⛔ ПОЧЕМУ ОТКАТ БЕЗОПАСЕН. Правило 1 контракта: `dictionary.flush()`
            # РАНЬШЕ первого UPDATE. Значит откат незафиксированной пачки оставляет
            # ячейку с ИСХОДНЫМ значением при живой записи словаря, а это ровно
            # случай Б классификации (`current == record.old_val`) -- повторный
            # прогон применит её заново. Обратная ситуация -- UPDATE зафиксирован,
            # а записи словаря нет -- невозможна по тому же правилу 1, и только она
            # ломала бы обратимость.
            ...
            for cell in cells:
                ...
                if outcome == "updated":
                    ...
                    skipped_empty += 1
                if updates and updates % _COMMIT_EVERY == 0:
                    conn.commit()
            conn.commit()
            return ApplyCounters(updates=updates, skipped_applied=skipped_applied,
                                  skipped_empty=skipped_empty, by_table=by_table)
        except BaseException:
            # ⛔ Откат ЯВНЫЙ, не «закроем соединение и как-нибудь». Незафиксированное
            # уходит целиком, зафиксированные пачки остаются -- и то и другое
            # доприменяется повторным прогоном (см. правило 1 выше).
            conn.rollback()
            raise
        finally:
            conn.close()
