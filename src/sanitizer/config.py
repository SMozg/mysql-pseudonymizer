...
class RunConfig:
    ...
    #: Замерено тремя точками: 0 — эха нет, разнообразие 42 %; 0.7 — эха нет,
    #: разнообразие 68 %; 1.0 — эхо непредсказуемо (14, затем 529, затем 558 из 600).
    #: ⛔ Эхо это отказ модели от задачи: не вытягивая её, она удовлетворяет
    #: требование различности самым дешёвым способом -- переписывает вход.
    #: ⛔ Плата названа честно: при температуре выше нуля два прогона с одним
    #: seed не совпадут побитово. ⛔ Приёмке это ничего не стоит: требование
    #: побитовой повторяемости снято 08.09.2026 как спор с собственным
    #: требованием «логичных замен». Разнообразие — прямая буква задания и видно
    #: в самой базе, поэтому выбрано оно.
