...
а не ветвление кода по классам.
  ...
  КОЛЬЦО (см. ПРАВИЛА-ОТКАЗ.md §4):
    ...
    4. фильтр -- ТРИ ЖЁСТКИХ проверки (лимит, свой исходный, занятость замены в
       классе, ``_passes_hard``); отказ у ВСЕХ кандидатов -> повтор НОВЫМ ПАКЕТОМ,
       потолок 3 повтора = 4 попытки, исчерпание -> RetriesExhausted. ⛔ Р-93
       (2026-09-04): совпадение с ЧУЖИМ исходным (универсум П) фильтр больше НЕ
       отклоняет -- среди жёстко прошедших кандидатов лишь ПРЕДПОЧИТАЕТ
       непересекающегося (``_collides_with_foreign_original``); число принятых
       пересечений публикует критерий 1 замером (в), НЕ гейтом. ⛔ Ревизия 08.09:
       занятость замены -- СНОВА ОТКАЗ (была предпочтением по Р-117, и критерий 12
       покраснел, см. ``_passes_hard``), с одним предохранителем: на ПОСЛЕДНЕЙ
       попытке бюджета склейка принимается, чтобы узкий пул не уронил прогон.
...
class Dictionary:
    ...
    def __init__(self, path, fernet: Fernet, passport):
        ...
        #: 📌 Не провал и не гейт, а число для критерия 26 -- мера того, насколько
        #: исчерпался пул поставщика. ⛔ Ревизия 08.09: склейка возможна ТОЛЬКО
        #: через предохранитель последней попытки (`_passes_hard`), поэтому число
        #: здесь читается прямо -- сколько значений пул не смог развести за все
        #: 4 попытки. Ноль -- норма, не удача.
        self._merges: dict = {}
    ...
    def _run_one_batch(self, cls: str, chunk: list, rule, provider, results: list,
        ...
        # 📌 ЗАГЛУШКИ ОТКЛЮЧЕНЫ решением владельца 07.09. Повторный заход отличается
        # от первого РОВНО ОДНИМ -- в нём меньше строк. Ни служебных строк, ни приписок
        # «уже отбито»: запрос на повторе тот же самый, только короче.
        # ⛔ ЧТО ЭТИМ СНЯТО, названо честно. Заглушки по краям ловили поставщика,
        # теряющего ровно N первых или последних элементов ответа: на пакете размером
        # ≤N он терял ВСЁ, и повтор из тех же строк воспроизводил ту же потерю --
        # неподвижная точка, попытки выгорали впустую. Теперь такой сбой формата
        # упрётся в потолок повторов и остановит прогон громко, назвав причину
        # («ответа нет» в разборе), а не будет обойдён молча.
        # 📌 Довод владельца: служебные строки уходили В ЗАПРОС К МОДЕЛИ, и она на них
        # отвечала -- в ответах были ключи __PAD__. Платить чужой моделью за защиту от
        # гипотетического сбоя, искажая при этом настоящий запрос, -- плохая сделка.
        ...
        for it in chunk:
            ...
            # 📌 ПРЕДОХРАНИТЕЛЬ (ревизия 08.09). Занятость замены снова ОТКАЗ
            # (`_passes_hard`), и это правильно: повтор солит хеш номером попытки
            # и сам даёт другое значение. Но на ПОСЛЕДНЕЙ попытке бюджета отказ
            # обернулся бы `RetriesExhausted` -- узкий пул значений уронил бы
            # прогон там, где решение есть. На ней склейка принимается.
            # ⛔ «Последняя попытка» берётся из бюджета ПО ЗНАЧЕНИЮ, а не из
            # `it["attempt"]`: бюджет считается по `value_key` (см. ниже, ревизия
            # дефекта 2), и у классов «по ячейке» несколько РАЗНЫХ ячеек делят
            # одно исходное значение -- по `it["attempt"]` предохранитель
            # взводился бы у каждой своим счётом, то есть не на последней
            # попытке бюджета, а раньше или позже неё.
            allow_merge = self._value_attempts.get(it["value_key"], 0) >= rule.retry_limit
            hard_passed = []
            for candidate in candidates:
                passed = self._passes_hard(cls, candidate, it, allow_merge=allow_merge)
                if passed is not None:
                    hard_passed.append(passed)
            # 📌 ЛЕСТНИЦА ПРЕДПОЧТЕНИЙ схлопнута до ДВУХ ступеней (Р-93). Занятость
            # из неё ушла в отказ выше, и предпочтением осталось ровно одно --
            # «не пересекается с ЧУЖИМ исходным». Ни одна ступень не отказ:
            # отказать вправе только жёсткие проверки.
            accepted = None
            for avoid_foreign in (True, False):
                ...
                if accepted is not None:
                    break
            # 📌 Диагностика, не гейт: теперь склейка возможна ТОЛЬКО через
            # предохранитель, поэтому это число -- прямая мера исчерпания пула.
            ...
            if accepted is None:
                ...
                if fails > rule.retry_limit:
                    ...
                    for tried in it["rejected"]:
                        why = self._hard_reason(cls, tried, it, allow_merge=allow_merge)
                        reasons[why] = reasons.get(why, 0) + 1
                    breakdown = ", ".join(
                        f"{why}: {n}" for why, n in sorted(reasons.items(), key=lambda kv: -kv[1])
                    ) or "кандидатов не было вовсе"
                    # 📌 РЕШАЮЩЕЕ ЧИСЛО -- сколько кандидатов пришло в ПОСЛЕДНЮЮ
                    # попытку. Ноль означает, что поставщик просто не ответил про
                    # эту строку, и это СОВСЕМ другая болезнь, чем «все кандидаты
                    # отбиты»: лечится она не пулом замен, а размером заявки.
                    # Без этого числа две болезни неразличимы по тексту остановки.
                    raise RetriesExhausted(
                        f"класс {cls}: попыток на значение {fails} -- потолок "
                        f"{rule.retry_limit} исчерпан (ключ {_cell_str(it['cell'])}); "
                        f"в последней попытке кандидатов пришло {len(candidates)}; "
                        f"за все попытки отбито {len(it['rejected'])} -- {breakdown}")
    ...
    def _hard_reason(self, cls: str, candidate: Any, it: Mapping, *,
                     allow_merge: bool = False) -> str:
        """Какая жёсткая проверка отбила кандидата -- ОДНИМ словом.

        📌 ЗЕРКАЛО `_passes_hard`, и это требование, а не пожелание. Замер 06.09:
        функция причин осталась со старой логикой и рапортовала «замена уже
        занята» после того, как занятость перестала быть отказом (Р-117) --
        разбор указывал на причину, которой уже не существовало, и увёл
        расследование в сторону. Диагностика, разошедшаяся с проверяемым кодом,
        хуже её отсутствия: она врёт уверенно.
        ⛔ Поэтому ревизия 08.09, вернувшая занятость в отказ, ТЕМ ЖЕ изменением
        вернула её сюда -- вместе с ``allow_merge``, тем же ключом и в том же
        порядке проверок. Меняешь политику отказа -- меняешь диагностику.
        📌 Наружу идёт ИМЯ причины, никогда само значение (критерий 23).
        """
        ...
        if isinstance(candidate, str):
            ...
            if _norm(candidate) == _norm(it["current"]):
                return "совпал со своим исходным"
            if not allow_merge and self._collides_with_taken(cls, candidate):
                return "замена уже занята"
            return "прошёл жёсткие проверки"
        if bytes(candidate) == it["current"]:
            return "совпал со своим исходным"
        if not allow_merge and self._collides_with_taken(cls, candidate):
            return "замена уже занята"
        return "прошёл жёсткие проверки"

    def _passes_hard(self, cls: str, candidate: Any, it: Mapping, *,
                     allow_merge: bool = False) -> Any:
        """📌 Ревизия 08.09: жёстких проверок ТРИ, отказ БЕЗ права предпочтения.

        Длиннее лимита колонки · равен СВОЕМУ исходному значению ЭТОЙ ячейки
        (``it["current"]``) · ЗАНЯТ другим исходным значением этого класса
        (склейка, ``_collides_with_taken``). Первые две -- о самой ячейке и
        неотменяемы: замена, не влезающая в колонку, не запишется, а равная
        своему исходному не является заменой вовсе.
        ⛔ ТРЕТЬЯ ВЕРНУЛАСЬ В ОТКАЗ, и цена её мягкости замерена. Р-117 сделал
        занятость лишь НЕпредпочтением; замер 08.09 показал, чем это кончается:
        критерий 12 (гейт кардинальности КАЖДОГО столбца, Р-127) покраснел на
        двух колонках -- `address.postal_code` 597->596 и `address.district`
        378->377. Детерминированный поставщик выдал ОДНО значение на две разные
        ячейки, и лестница предпочтений приняла склейку молча. Два правила
        противоречили друг другу: Р-127 гейтит кардинальность столбца, Р-117
        разрешал её терять без единого повтора.
        📌 Отказ здесь дёшев ровно потому, что повтор сам даёт другое значение:
        детерминированный поставщик солит хеш НОМЕРОМ ПОПЫТКИ
        (`providers/generator.py::_digest`, `providers/nontext.py::_rng`), и
        одного повтора хватает -- новый пул не нужен.
        📌 ``allow_merge`` -- ПРЕДОХРАНИТЕЛЬ, и только он: на ПОСЛЕДНЕЙ попытке
        бюджета склейка всё-таки принимается. Без него узкий пул значений упёрся
        бы в `RetriesExhausted` и уронил прогон там, где решение существует. Так
        склейка становится ИСХОДОМ, а не нормой, -- ровно то, чего словами
        требует критерий 26.
        📌 Предпочтением осталась РОВНО ОДНА бывшая проверка: совпадение с ЧУЖИМ
        исходным (Р-93, критерий 1в) -- тот запрет был строже задачи.
        """
        if isinstance(candidate, str):
            limit = it["field_rule"].length_limit
            if limit is not None and len(candidate) > limit:
    ...
    def _collides_with_taken(self, cls: str, candidate: Any) -> bool:
        ...
        ⛔ Ревизия 08.09: ОТКАЗ, а не предпочтение. Стоит склейка кардинальности
        столбца, а её гейтит критерий 12 (Р-127) -- значит мягкость здесь красит
        приёмку (замер 08.09: `address.postal_code` 597->596, `address.district`
        378->377). Отказ дёшев: повтор солит хеш номером попытки и сам разводит
        значения.
        📌 Что от Р-117 осталось верным: склейка не ломает ни личность, ни
        обратимость -- личность несёт первичный ключ (критерий 18), запись словаря
        заведена на ячейку, тёзки существуют и в жизни. Поэтому предохранитель на
        последней попытке принимает её, а не останавливает прогон; число принятых
        публикует критерий 26.
        """
        if isinstance(candidate, str):
