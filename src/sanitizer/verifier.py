...
class _AcceptanceReport(AcceptanceReport):
    ...
    def render_values_only(self) -> str:
        """Части отчёта, которые МОГУТ нести значения базы -- вход сторожа (критерий 23).

        ⛔ ЧТО СЮДА ВХОДИТ и почему именно это:
          · разрывы сквозной замены -- единственное место, где отчёт когда-то печатал
            НАСТОЯЩЕЕ исходное значение (Р-89); сейчас там ключ сущности, но место
            остаётся value-несущим по смыслу, и сторож обязан его стеречь;
          · класс общедоступных значений -- туда пишется основание НА КОЛОНКУ, текст
            человеческий, и однажды в него можно вписать пример значения.
        ⛔ ЧТО СЮДА НЕ ВХОДИТ: счётчики прогона, строка расхода, замеры критериев,
        своды, коды. Это числа, КОТОРЫЕ ИНСТРУМЕНТ ВЫЧИСЛИЛ САМ, а не прочитал из
        базы. Сравнивать их со значениями -- сравнивать яблоки с грушами; так и
        родилась ложная тревога 07.09.
        📌 Сторож от этого НЕ становится беззубым: тест `test_c23_catches_a_value_
        planted_into_the_report` сажает настоящее значение в разрыв и требует красного.
        """
        parts = []
        for row in self.breaks_rows:
            parts.append(f"{row.check} {row.expect} {row.fact}")
        for row in self.pub_section:
            parts.append(f"{row.table} {row.column} {row.ground}")
        return "\n".join(parts)
...
class Verifier:
    ...
    def accept(self) -> AcceptanceReport:
        ...
        try:
            ...
            # ⛔ Сторожу отдаётся НЕ весь текст, а только те части отчёта, которые
            # МОГУТ нести значения базы. Причина -- в происхождении: число в отчёте
            # инструмент ВЫЧИСЛИЛ сам, значение он ПРОЧИТАЛ из базы, и сравнивать их
            # одной меркой нельзя. Инцидент 07.09: счётчик «из 5027 выданных замен»
            # совпал с почтовым индексом 5027 из этой же базы, и сторож закричал
            # «утечка» там, где её нет. Порог по длине числа лечил бы симптом.
            final_c23 = self._c23(report_text=report.render_values_only())
    ...
    def _c12(self, after, conn) -> CriterionResult:
        """📌 Р-127: гейт -- КАРДИНАЛЬНОСТЬ КАЖДОГО СТОЛБЦА И различность пар. ОБЕ.

        ⛔ История этой меры -- урок, который стоит держать на виду. Сначала она
        гейтила кардинальность столбца. Затем (Р-117) её смягчили до пар «имя +
        фамилия»: живая модель схлопывала столбцы, а личность действительно несёт
        первичный ключ, и тёзки в жизни есть. Формально верно -- и ровно поэтому
        опасно: независимый проверяющий развернул поставку, увидел 37 различных
        имён на 599 клиентов при зелёном критерии и назвал это своими словами --
        «мера подстроена под результат». Он прав. Заказчик просил сохранить
        РАЗНООБРАЗИЕ, а не только различность личностей.

        📌 Мера возвращена к строгой, потому что стала ВЫПОЛНИМОЙ: имя и фамилия
        обслуживаются перестановкой (Р-123, Р-126), и кардинальность там точна по
        построению. ⛔ Смягчать требование, пока оно невыполнимо, и ужесточать
        обратно, когда научился, -- честный порядок; обратный порядок -- подгонка.
        """
        ...
        return _cr(12, "Разнообразие: кардинальность каждого столбца не упала И различных "
                        "пар «имя + фамилия» столько же",
                    "ни одна колонка не потеряла различных значений; число различных пар "
                    "не упало ни в customer, ни в staff",
                    fact, ok)
    ...
    def _c23(self, report_text: Optional[str] = None) -> CriterionResult:
        ...
        # ⛔ ПРОИСХОЖДЕНИЕ, А НЕ ДЛИНА. Сюда приходит НЕ весь текст отчёта, а только
        # его value-несущие части (`render_values_only`): счётчики и замеры инструмент
        # вычислил сам, и искать в них значения базы -- сравнивать яблоки с грушами.
        # 📌 Инцидент 07.09: счётчик «из 5027 выданных замен» совпал с почтовым
        # индексом 5027, и сторож объявил утечку там, где её нет. Порог по длине числа
        # («короткие не считаем») лечил бы симптом и ослабил бы сторожа по-настоящему.
        # ⛔ Целыми лексемами, не вхождением подстроки: замены длиной 1-2 знака иначе
        # ловятся внутри сводов и внутри слова STRICT_TRANS_TABLES.
