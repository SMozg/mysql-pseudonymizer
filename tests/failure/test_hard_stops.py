...
def test_collision_inside_one_answer_costs_cardinality_not_identity(conn, case_pipeline):
    """Условие 3 пакетирования при Р-117: одинаковые значения в одном ответе
    больше не отбиваются, а обходятся ценой кардинальности -- и только её.

    📌 Раньше этот тест требовал, чтобы внутрипакетная коллизия была РАЗБИТА и
    число различных имён осталось прежним. Требование отменено вместе с жёсткой
    проверкой занятости (Р-117): личность несёт первичный ключ, а запись словаря
    заведена на ячейку, поэтому общая замена не склеивает людей.
    📌 Что тест требует теперь -- ровно то, что осталось обязательным:
      1. прогон НЕ падает: поставщик, отдавший дубли, не повод для остановки;
      2. кардинальность столбца просела -- и это видно числом, а не спрятано;
      3. 📌 ГРАНИЦА Р-117 НАЗВАНА ЧЕСТНО: неисправный поставщик, отдающий дубли
         и по именам, и по фамилиям, СПОСОБЕН схлопнуть личности -- пар станет
         меньше, чем строк. Раньше это ловил фильтр отказом посреди прогона;
         теперь ловит критерий 12 на приёмке, и красным. Тест закрепляет, что
         сигнал не потерян, а переехал: он обязан быть ВИДЕН.
    """
    provider = fakes.FakeModelProvider(mode=fakes.MODE_INTRA_COLLISION)
    case_pipeline.run(work_schema=case_pipeline.schema, provider=provider)

    names = h.scalar(conn, h.q(
        "SELECT COUNT(DISTINCT first_name) n FROM {cur}.customer",
        cur=case_pipeline.schema))
    assert names < R.C12_DISTINCT_AFTER["customer.first_name"], (
        "поставщик отдавал дубли, а кардинальность не просела -- значит "
        "внутрипакетная коллизия где-то всё ещё отбивается")

    rows_total = h.scalar(conn, h.q(
        "SELECT COUNT(*) n FROM {cur}.customer", cur=case_pipeline.schema))
    pairs = h.scalar(conn, h.q(
        "SELECT COUNT(DISTINCT first_name, last_name) n FROM {cur}.customer",
        cur=case_pipeline.schema))
    assert pairs < rows_total, (
        "поставщик отдавал дубли по обоим классам, а пары не просели -- значит "
        "критерий 12 нечего было бы ловить, и тест перестал бы что-либо доказывать")
    # 📌 И вот ради чего всё это: просадку пар ОБЯЗАН увидеть критерий 12.
    # Пока сигнал виден числом, решение Р-117 остаётся безопасным: склейка
    # внутри столбца дозволена, схлопывание личностей -- нет, и его видно.
    assert pairs < R.C12_DISTINCT_AFTER["customer.first_name"] + rows_total, (
        "счёт пар вышел за пределы возможного -- проверка меряет не то")
