...
def test_collision_inside_one_answer_is_refused_and_the_retry_undoes_it(conn, case_pipeline):
    """Условие 3 пакетирования: два элемента ОДНОГО ответа с одной заменой --
    отказ по второй ячейке, повтор разводит значения, кардинальность цела.

    ⛔ ЭТОТ ТЕСТ ПЕРЕПИСАН, А НЕ ПОДОГНАН, и вот журнал его утверждений.
      Было (до Р-117): коллизия РАЗБИВАЕТСЯ, кардинальность прежняя.
      Стало (Р-117):   коллизия ПРОХОДИТ ценой кардинальности; тест требовал,
                       чтобы число различных имён ПРОСЕЛО, и это требование
                       кодифицировало политику, а не свойство продукта.
      Стало (ревизия 08.09): снова как было -- и по замеру, а не по вкусу.
                       Р-117 стоил критерия 12 на поставке: `address.postal_code`
                       597->596 и `address.district` 378->377 при зелёном 26.
                       Занятость замены вернулась в ОТКАЗ (`_passes_hard`), склейка
                       проходит только предохранителем последней попытки.
    📌 Внутрипакетная коллизия под новой политикой обходится ДЁШЕВО и без потерь:
    первый элемент занимает замену, второй отбивается, повторный пакет собирается
    из одной этой ячейки -- двойник в нём дублей не делает (`len(items) >= 2`
    ложно), а боевой поставщик солит хеш номером попытки. Один повтор -- и
    значения разведены.
    ⛔ Проверка повтора обязательна: без неё тест зеленел бы и на поставщике,
    который дублей вовсе не отдавал, -- то есть не доказывал бы ничего.
    """
    ...
    for column in ("first_name", "last_name"):
        n = h.scalar(conn, h.q(
            f"SELECT COUNT(DISTINCT {column}) n FROM {{cur}}.customer",
            cur=case_pipeline.schema))
        assert n == R.C12_DISTINCT_AFTER[f"customer.{column}"], (
            f"customer.{column}: различных {n}, ждали "
            f"{R.C12_DISTINCT_AFTER[f'customer.{column}']} -- внутрипакетная коллизия "
            f"стоила кардинальности, хотя повтор был обязан её снять")

    worst = max(provider.asked.values())
    assert worst >= 2, (
        "ни одно значение не спрашивалось дважды -- значит коллизии не было вовсе, "
